...
export class AgentManager {
  ...
  async routeToAgent(
    ...
    // Start with general intake for new conversations
    if (!context.previousAgent) {
      return AgentType.GENERAL_INTAKE;
    }

    // Analyze input for routing signals
    const routingSignals = this.analyzeRoutingSignals(input.toLowerCase());

    // Check for emergency signals first
    if (routingSignals.isEmergency) {
      return AgentType.EMERGENCY_AFTER_HOURS;
    }

    // Spanish language preference
    if (routingSignals.needsSpanish) {
      return AgentType.SPANISH_LANGUAGE;
    }

    // Route based on legal area
    if (routingSignals.legalArea) {
      return this.getAgentForLegalArea(routingSignals.legalArea);
    }

    // Default to general intake
    return AgentType.GENERAL_INTAKE;
  }
