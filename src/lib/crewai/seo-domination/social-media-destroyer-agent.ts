...
`;
  ...
  private async postToFacebook(post: SocialPost): Promise<void> {
    ...
    try {
      const response = await fetch(`${apiUrl}/${pageId}/feed`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
  ...
  private async postToInstagram(post: SocialPost): Promise<void> {
    ...
    try {
      ...
      const containerResponse = await fetch(`${apiUrl}/${businessAccountId}/media`, {
        method: 'POST',
      ...
      const publishResponse = await fetch(`${apiUrl}/${businessAccountId}/media_publish`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
