  ...
  window.addEventListener('error', (event) => {
    ...
    if (event.target && event.target !== window) {
      ...
      if (isExternalResource) {
        event.preventDefault();
        console.warn('[External Resource Error Suppressed]', src);
        return;
      }
    }
...
function cleanupOrphanedGlobals() {
  ...
  if ('wb_async_messenger' in window) {
    try {
      ...
      console.info('[Cleanup] Removed orphaned wb_async_messenger');
    } catch (error) {
      // Some properties might be non-configurable
      console.warn('[Cleanup] Could not remove wb_async_messenger:', error);
    }
  }
  ...
  orphanedGlobals.forEach(globalName => {
    if (globalName in window && !(window as any)[globalName]?.loaded) {
      try {
        ...
        console.info(`[Cleanup] Removed orphaned ${globalName}`);
      } catch (error) {
