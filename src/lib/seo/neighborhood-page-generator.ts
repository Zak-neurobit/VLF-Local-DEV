...
export const RALEIGH_NEIGHBORHOODS = [
  ...
  { name: 'North Raleigh', slug: 'north-raleigh', zipCodes: ['27609', '27612', '27613', '27614', '27615'] },
...
function getNeighborhoodLandmarks(neighborhood: string, city: string): string[] {
  const landmarks: Record<string, Record<string, string[]>> = {
    Charlotte: {
      Uptown: ['Bank of America Stadium', 'Spectrum Center', 'Romare Bearden Park', 'BB&T Ballpark'],
      ...
      'Myers Park': ['Queens University', 'Myers Park Country Club', 'Myers Park Traditional School'],
      Ballantyne: ['Ballantyne Corporate Park', 'Ballantyne Village', 'Ballantyne Country Club'],
      // Add more neighborhoods
    },
    Raleigh: {
      Downtown: ['Red Hat Amphitheater', 'Raleigh Convention Center', 'City Market', 'Moore Square'],
    ...
    Durham: {
      Downtown: ['Durham Bulls Athletic Park', 'DPAC', 'American Tobacco Campus', 'Durham Central Park'],
      'Ninth Street': ['Ninth Street Shopping', 'Duke East Campus', 'Elmo\'s Diner'],
      // Add more neighborhoods
    }
  };
  
  return landmarks[city]?.[neighborhood] || ['Local shopping centers', 'Community parks', 'Schools', 'Major intersections'];
}

function getNeighborhoodDemographics(neighborhood: string): { population: string; medianIncome: string; diversity: string } {
...
function getNearbyCourtsAndServices(city: string): Array<{ name: string; distance: string; type: string }> {
