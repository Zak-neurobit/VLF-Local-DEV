...
export const POWER_WORDS = {
  trust: [
    'Trusted', 'Proven', 'Award-Winning', 'Elite', 'Premier', 'Top-Rated',
    'Certified', 'Accredited', 'Recognized', 'Leading', 'Renowned'
  ],
  urgency: [
    'Immediate', 'Emergency', 'Now', 'Today', 'Instant', 'Same-Day',
    'Fast', 'Rapid', 'Quick', 'Urgent', '24/7'
  ],
  results: [
    'Maximum', 'Winning', 'Successful', 'Guaranteed', 'Proven Results',
    'Record-Breaking', 'Highest', 'Best', 'Superior', 'Unmatched'
  ],
  expertise: [
    'Expert', 'Specialist', 'Master', 'Professional', 'Experienced',
    'Skilled', 'Qualified', 'Veteran', 'Senior', 'Board-Certified'
  ],
  aggression: [
    'Aggressive', 'Fighting', 'Fierce', 'Relentless', 'Tenacious',
    'Determined', 'Powerful', 'Strong', 'Bold', 'Fearless'
  ],
  value: [
    'Free', 'No-Cost', 'Affordable', 'No Fee Unless We Win', 'Risk-Free',
    'Complimentary', 'Budget-Friendly', 'Cost-Effective'
  ],
  locality: [
    'Local', 'Nearby', 'Area', 'Regional', 'Community', 'Neighborhood',
    'Downtown', 'North Carolina\'s', 'NC\'s', 'Serving'
  ]
};
...
export function generateOptimizedTitle(
  ...
  if (typeof templates === 'object' && !Array.isArray(templates)) {
    const subTemplates = templates[subType as keyof typeof templates];
...
export function generateOptimizedDescription(
  ...
  // Add opening
  const opening = DESCRIPTION_ENHANCERS.openings[
  ...
  if (pageType === 'practiceArea') {
    parts.push(`NC's most ${POWER_WORDS.aggression[0].toLowerCase()} ${variables.service || 'legal'} team.`);
  } else if (pageType === 'location') {
    parts.push(`${variables.city || 'North Carolina'}'s ${POWER_WORDS.trust[0].toLowerCase()} law firm.`);
  } else {
    parts.push('North Carolina\'s premier legal team.');
  }
  
  // Add credibility
  const credibility = DESCRIPTION_ENHANCERS.credibility[
    Math.floor(Math.random() * DESCRIPTION_ENHANCERS.credibility.length)
  ];
  parts.push(credibility + '.');
  
  // Add local trust with variables
  let localTrust = DESCRIPTION_ENHANCERS.localTrust[
  ...
  // Add urgency
  const urgency = DESCRIPTION_ENHANCERS.urgency[
    Math.floor(Math.random() * DESCRIPTION_ENHANCERS.urgency.length)
  ];
  parts.push(urgency + '.');
  
  // Add call to action
  const cta = DESCRIPTION_ENHANCERS.callToAction[
...
export function generateABTestMetadata(
  ...
  const selectedVariant = variant || (['A', 'B', 'C'][Math.floor(Math.random() * 3)] as 'A' | 'B' | 'C');
