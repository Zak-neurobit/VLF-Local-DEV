...
export function generateRecurringConsultationEvents(
  ...
  for (let i = 0; i < recurrence.count; i++) {
    ...
    events.push(generateConsultationEventSchema({
      ...baseEvent,
...
export const CONSULTATION_EVENT_TYPES = {
  immigration: {
    ...
    description: 'Get immediate help with deportation defense, green cards, citizenship, work visas, and all immigration matters. Our aggressive attorneys fight ICE and USCIS to protect your American dream. Same-day appointments available. Se habla español.',
    practiceArea: 'Immigration Law'
  },
  personalInjury: {
    name: 'FREE Personal Injury Case Review - Maximum Compensation Guaranteed',
    description: 'Injured in an accident? Our elite attorneys fight insurance companies to get you MAXIMUM compensation. Car accidents, truck crashes, slip & fall, medical malpractice. No fee unless we win. Get help NOW!',
    practiceArea: 'Personal Injury'
  },
  criminalDefense: {
    name: 'FREE Criminal Defense Consultation - Former Prosecutors Fighting for YOU',
    description: 'Arrested? Charged? Our aggressive criminal defense team includes former prosecutors who know how to WIN. DWI/DUI, drug charges, assault, theft. Available 24/7 for jail release. Protect your freedom NOW!',
    practiceArea: 'Criminal Defense'
  },
  workersComp: {
    name: 'FREE Workers Compensation Consultation - Get Every Dollar You Deserve',
    description: 'Hurt at work? Employer denying your claim? We FIGHT for injured workers and get MAXIMUM benefits. Medical bills, lost wages, permanent disability. No upfront costs. Call before you lose your rights!',
    practiceArea: 'Workers Compensation'
  },
  general: {
    name: 'FREE Legal Consultation - North Carolina\'s Most Aggressive Law Firm',
    description: 'Need legal help? Get a FREE consultation with NC\'s most aggressive attorneys. Immigration, personal injury, criminal defense, workers comp. 60+ years experience. 30,000+ cases won. Available 24/7!',
...
export function generateOfficeConsultationEvents(
  ...
  for (let i = 0; i < 5; i++) { // Monday-Friday
    ...
    events.push({
      ...baseEvent,
      name: `${baseEvent.name} - ${office.city} Office`,
...
export function generateEmergencyConsultationEvent(): WithContext<Event> {
  ...
  return generateConsultationEventSchema({
    ...
    description: 'URGENT legal emergency? Arrested? Facing deportation? Injured? Our emergency response team is available 24/7/365. Get IMMEDIATE help from NC\'s most aggressive attorneys. Call NOW: 1-844-YO-PELEO',
...
export function generateSpecialEventSchema(
  eventData: {
