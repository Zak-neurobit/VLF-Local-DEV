...
export function useSafeDOM() {
  ...
  const safeQuerySelector = useCallback(<T extends Element>(
    selector: string,
  ...
  const safeQuerySelectorAll = useCallback(<T extends Element>(
    selector: string,
    parent: ParentNode = document
  ): NodeListOf<T> | [] => {
  ...
  const safeCreateElement = useCallback(<K extends keyof HTMLElementTagNameMap>(
    tagName: K,
  ...
  const safeCloneNode = useCallback(<T extends Node>(
    node: T | null,
    deep?: boolean
  ): T | null => {
    if (!isMountedRef.current || !isHydratedRef.current) return null;
