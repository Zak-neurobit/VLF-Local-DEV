...
class DeploymentTester {
  ...
  private async testParallelProcessing(): Promise<void> {
    ...
    const promises = testContexts.map(async (context, i) => {
      return this.orchestrator.routeMessage(
        `Test message ${i} for parallel processing`,
        context
      );
    });
    ...
    logger.info(`Parallel processing completed: ${successful} successful, ${failed} failed in ${executionTime}ms`);
  ...
  private async testInterAgentCommunication(): Promise<void> {
    ...
    // Test broadcasting
    await this.coordinator.broadcastMessage(
      'legal-consultation',
      'status-update',
      {
  ...
  private async testMemorySystem(): Promise<void> {
    ...
    this.orchestrator.updateAgentMemory(
      'legal-consultation',
      'test-key',
      { value: 'test-value', timestamp: Date.now() }
    );
