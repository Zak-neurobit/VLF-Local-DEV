...
const MetricCard: React.FC<MetricCardProps> = ({ 
  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg p-6 border-l-4 ${color} transition-all duration-300 hover:shadow-xl`}>
...
const LivingMetrics: React.FC = () => {
  ...
  const metricConfigs = [
    {
      ...
      trend: data.metrics.visitorCount > previousMetrics.visitorCount ? 'up' : 'stable'
    },
    {
      ...
      trend: data.metrics.conversationsActive > previousMetrics.conversationsActive ? 'up' : 'stable'
    },
    {
      title: 'Reviews Today',
      ...
      trend: data.metrics.reviewsToday > previousMetrics.reviewsToday ? 'up' : 'stable'
    },
    {
      ...
      trend: data.metrics.contentCreated > previousMetrics.contentCreated ? 'up' : 'stable'
    },
    {
      ...
      trend: data.metrics.rankingChanges > previousMetrics.rankingChanges ? 'up' : 'stable'
    },
    {
      ...
      trend: data.metrics.socialEngagement > previousMetrics.socialEngagement ? 'up' : 'stable'
    },
    {
      ...
      trend: data.metrics.leadGeneration > previousMetrics.leadGeneration ? 'up' : 'stable'
    },
    {
      ...
      trend: data.metrics.conversionRate > previousMetrics.conversionRate ? 'up' : 'stable'
    }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ${pulseAnimation ? 'animate-pulse' : ''}`}>
        {metricConfigs.map((config, index) => (
          <MetricCard
            key={config.title}
            title={config.title}
            value={config.value}
            previousValue={(previousMetrics as any)[Object.keys(data.metrics)[index]]}
