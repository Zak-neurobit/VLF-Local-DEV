...
const DynamicHomepage: React.FC<DynamicHomePageProps> = ({ language }) => {
  ...
  const [latestBlogPosts, setLatestBlogPosts] = useState<Array<{ title: string; slug: string; date: string }>>([]);
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          try {
            const response = await fetch(`/api/location?lat=${position.coords.latitude}&lon=${position.coords.longitude}`);
