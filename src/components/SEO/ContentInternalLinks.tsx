...
import { generateContextualLinks, PRACTICE_AREAS, NC_CITIES } from '@/lib/seo/internal-linking-mesh';
...
const KEYWORD_MAPPINGS = [
  ...
  { keywords: ['immigration', 'green card', 'visa', 'deportation'], link: '/practice-areas/immigration' },
  { keywords: ['personal injury', 'accident', 'injured', 'injury'], link: '/practice-areas/personal-injury' },
  { keywords: ['criminal', 'arrest', 'charged', 'crime'], link: '/practice-areas/criminal-defense' },
  { keywords: ['workers comp', 'work injury', 'workplace accident'], link: '/practice-areas/workers-compensation' },
  ...
  // Service keywords
  { keywords: ['car accident', 'auto accident', 'vehicle crash'], link: '/practice-areas/personal-injury/car-accidents' },
  { keywords: ['truck accident', '18-wheeler'], link: '/practice-areas/personal-injury/truck-accidents' },
  { keywords: ['dwi', 'dui', 'drunk driving'], link: '/practice-areas/criminal-defense/dwi-drunk-driving' },
  { keywords: ['citizenship', 'naturalization'], link: '/practice-areas/immigration/citizenship-naturalization' },
...
export function ContentInternalLinks({ 
  ...
  const addInternalLinks = (text: string): React.ReactNode => {
    ...
    const sortedMappings = [...KEYWORD_MAPPINGS].sort((a, b) => 
    ...
    sortedMappings.forEach(mapping => {
      ...
      mapping.keywords.forEach(keyword => {
        ...
        if (match && match.index !== undefined) {
          ...
          const overlaps = addedLinks.some(link => 
            (match.index >= link.start && match.index < link.end) ||
            (match.index + match[0].length > link.start && match.index + match[0].length <= link.end)
          );
  ...
  const contextualLinks = currentPage 
...
export function AutoLinkContent({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  const processNode = (node: React.ReactNode): React.ReactNode => {
    ...
    if (Array.isArray(node)) {
      return node.map((child, index) => <React.Fragment key={index}>{processNode(child)}</React.Fragment>);
    }
  ...
  return (
    <div className={`auto-linked-content ${className}`}>
