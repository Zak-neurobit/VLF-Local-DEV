...
export const MinimalRetellClient: React.FC<MinimalRetellClientProps> = ({
  ...
  const startCall = async () => {
    ...
    if (isCallInProgressRef.current) {
      console.log('Call already in progress for this user');
      return;
    }
    ...
    try {
      ...
      client.on('update', (update: any) => {
        ...
        console.log('Retell update received:', JSON.stringify(update, null, 2));
        ...
        if (update.transcript) {
          ...
          if (transcriptText && transcriptText.trim() && transcriptText !== lastTranscriptRef.current) {
            console.log('New transcript text:', transcriptText);
        ...
        if (update.response) {
          ...
          if (responseText && responseText.trim() && responseText !== lastResponseRef.current) {
            console.log('New response text:', responseText);
        ...
        if (update.type === 'transcript' && update.text) {
          ...
          if (text && text !== lastTranscriptRef.current) {
            console.log('Alternative transcript format:', text);
        ...
        if (update.type === 'response' && update.text) {
          ...
          if (text && text !== lastResponseRef.current) {
            console.log('Alternative response format:', text);
      ...
      console.log('Call started, now starting audio playback...');
      ...
      setTimeout(async () => {
        try {
          if (retellClientRef.current) {
            ...
            console.log('✅ Audio playback started - user joined the call');
          }
        } catch (audioError) {
