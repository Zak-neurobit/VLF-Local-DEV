import { Metadata } from 'next';
import Link from 'next/link';

export const metadata: Metadata = {
  title: 'Accidentes de Motocicleta - Vasquez Law Firm, PLLC',
  description:
    'Lesionado en un accidente de motocicleta? Nuestros abogados en Raleigh, NC luchan por la compensación que merece. ¡Contáctenos hoy para una consulta gratis!',
  openGraph: {
    title: 'Accidentes de Motocicleta - Vasquez Law Firm, PLLC',
    description:
      'Lesionado en un accidente de motocicleta? Nuestros abogados en Raleigh, NC luchan por la compensación que merece. ¡Contáctenos hoy para una consulta gratis!',
    images: [
      {
        url: '../../../../wp-content/uploads/2024/04/johnston-county-motorcycle-accident-injury-attorneys-1.jpg',
      },
    ],
  },
};

export default function AccidentesMotocicletaPage() {
  return (
    <div className="min-h-screen bg-white">
      {/* Hero Section */}
      <section className="bg-gradient-to-r from-burgundy-900 to-burgundy-700 text-white py-16">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-6">Accidentes de Motocicleta</h1>
            <p className="text-xl"></p>
          </div>
        </div>
      </section>

      {/* Main Content */}
      <section className="py-12">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <div className="prose prose-lg max-w-none"></div>

            {/* Internal Links */}
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="bg-gray-50 py-12">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-3xl font-bold text-burgundy-900 mb-4">Ready to Get Started?</h2>
            <p className="text-xl text-gray-700 mb-8">
              Contact us today for a free consultation with our experienced attorneys.
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link
                href="/contact"
                className="bg-burgundy-700 text-white px-8 py-3 rounded-md hover:bg-burgundy-800 transition-colors"
              >
                Schedule Consultation
              </Link>
              <a
                href="tel:+18449673536"
                className="bg-gold-500 text-burgundy-900 px-8 py-3 rounded-md hover:bg-gold-600 transition-colors"
              >
                Call 1-844-YO-PELEO
              </a>
            </div>
          </div>
        </div>
      </section>

      {/* Schema Markup */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify([
            {
              '@context': 'https://schema.org',
              '@graph': [
                {
                  '@type': 'WebPage',
                  '@id':
                    'https://www.vasquezlawnc.com/es/areas-de-practica/lesiones-personales/accidentes-motocicleta/',
                  url: 'https://www.vasquezlawnc.com/es/areas-de-practica/lesiones-personales/accidentes-motocicleta/',
                  name: 'Accidentes de Motocicleta - Vasquez Law Firm, PLLC',
                  isPartOf: { '@id': 'https://www.vasquezlawnc.com/#website' },
                  primaryImageOfPage: {
                    '@id':
                      'https://www.vasquezlawnc.com/es/areas-de-practica/lesiones-personales/accidentes-motocicleta/#primaryimage',
                  },
                  image: {
                    '@id':
                      'https://www.vasquezlawnc.com/es/areas-de-practica/lesiones-personales/accidentes-motocicleta/#primaryimage',
                  },
                  thumbnailUrl:
                    '../../../../wp-content/uploads/2024/04/johnston-county-motorcycle-accident-injury-attorneys-1.jpg',
                  datePublished: '2024-04-05T19:05:08+00:00',
                  dateModified: '2025-02-20T11:11:33+00:00',
                  description:
                    'Lesionado en un accidente de motocicleta? Nuestros abogados en Raleigh, NC luchan por la compensación que merece. ¡Contáctenos hoy para una consulta gratis!',
                  breadcrumb: {
                    '@id':
                      'https://www.vasquezlawnc.com/es/areas-de-practica/lesiones-personales/accidentes-motocicleta/#breadcrumb',
                  },
                  inLanguage: 'en-US',
                  potentialAction: [
                    {
                      '@type': 'ReadAction',
                      target: [
                        'https://www.vasquezlawnc.com/es/areas-de-practica/lesiones-personales/accidentes-motocicleta/',
                      ],
                    },
                  ],
                },
                {
                  '@type': 'ImageObject',
                  inLanguage: 'en-US',
                  '@id':
                    'https://www.vasquezlawnc.com/es/areas-de-practica/lesiones-personales/accidentes-motocicleta/#primaryimage',
                  url: '../../../../wp-content/uploads/2024/04/johnston-county-motorcycle-accident-injury-attorneys-1.jpg',
                  contentUrl:
                    '../../../../wp-content/uploads/2024/04/johnston-county-motorcycle-accident-injury-attorneys-1.jpg',
                  width: 1000,
                  height: 400,
                  caption:
                    'Abogados de Lesiones por Accidentes de Motocicleta del Condado de Johnston',
                },
                {
                  '@type': 'BreadcrumbList',
                  '@id':
                    'https://www.vasquezlawnc.com/es/areas-de-practica/lesiones-personales/accidentes-motocicleta/#breadcrumb',
                  itemListElement: [
                    {
                      '@type': 'ListItem',
                      position: 1,
                      name: 'Home',
                      item: 'https://www.vasquezlawnc.com/',
                    },
                    {
                      '@type': 'ListItem',
                      position: 2,
                      name: 'Español',
                      item: 'https://www.vasquezlawnc.com/es/',
                    },
                    {
                      '@type': 'ListItem',
                      position: 3,
                      name: 'Abogados que prestan servicios en Raleigh, Charlotte y Smithfield, NC',
                      item: 'https://www.vasquezlawnc.com/es/areas-de-practica/',
                    },
                    {
                      '@type': 'ListItem',
                      position: 4,
                      name: 'Abogados de Lesiones Personales en Raleigh, NC',
                      item: 'https://www.vasquezlawnc.com/es/areas-de-practica/lesiones-personales/',
                    },
                    { '@type': 'ListItem', position: 5, name: 'Accidentes de Motocicleta' },
                  ],
                },
                {
                  '@type': 'WebSite',
                  '@id': 'https://www.vasquezlawnc.com/#website',
                  url: 'https://www.vasquezlawnc.com/',
                  name: 'Vasquez Law Firm, PLLC',
                  description: 'Raleigh, NC Immigration Attorney',
                  potentialAction: [
                    {
                      '@type': 'SearchAction',
                      target: {
                        '@type': 'EntryPoint',
                        urlTemplate: 'https://www.vasquezlawnc.com/?s={search_term_string}',
                      },
                      'query-input': {
                        '@type': 'PropertyValueSpecification',
                        valueRequired: true,
                        valueName: 'search_term_string',
                      },
                    },
                  ],
                  inLanguage: 'en-US',
                },
              ],
            },
            {
              '@context': 'https://schema.org',
              '@type': 'LegalService',
              '@id': 'https://vasquezlawnc.com/#organization',
              name: 'Vasquez Law Firm, PLLC',
              alternateName: 'Vasquez Law',
              url: 'https://vasquezlawnc.com',
              logo: 'https://vasquezlawnc.com/images/logo.png',
              sameAs: [
                'https://www.facebook.com/vasquezlawfirm',
                'https://twitter.com/vasquezlawfirm',
                'https://www.linkedin.com/company/vasquez-law-firm',
              ],
              contactPoint: {
                '@type': 'ContactPoint',
                telephone: '+1-844-967-3536',
                contactType: 'customer service',
                availableLanguage: ['English', 'Spanish'],
              },
            },
            {
              '@context': 'https://schema.org',
              '@type': 'BreadcrumbList',
              itemListElement: [
                {
                  '@type': 'ListItem',
                  position: 1,
                  name: 'Home',
                  item: 'https://vasquezlawnc.com',
                },
                {
                  '@type': 'ListItem',
                  position: 2,
                  name: 'Es',
                  item: 'https://vasquezlawnc.com/es',
                },
                {
                  '@type': 'ListItem',
                  position: 3,
                  name: 'Areas de practica',
                  item: 'https://vasquezlawnc.com/es/areas-de-practica',
                },
                {
                  '@type': 'ListItem',
                  position: 4,
                  name: 'Lesiones personales',
                  item: 'https://vasquezlawnc.com/es/areas-de-practica/lesiones-personales',
                },
                {
                  '@type': 'ListItem',
                  position: 5,
                  name: 'Accidentes motocicleta',
                  item: 'https://vasquezlawnc.com/es/areas-de-practica/lesiones-personales/accidentes-motocicleta',
                },
              ],
            },
            {
              '@context': 'https://schema.org',
              '@type': 'LegalService',
              name: 'Vasquez Law Firm',
              aggregateRating: {
                '@type': 'AggregateRating',
                ratingValue: '4.8',
                reviewCount: '127',
                bestRating: '5',
              },
            },
          ]),
        }}
      />
    </div>
  );
}
