...
async function handleChatPOST(request: NextRequest) {
  try {
    ...
    if (!hasOpenAI && !hasGHL) {
      logger.error('Neither OpenAI nor GHL are configured');
      return NextResponse.json({
        response:
          language === 'es'
            ? 'Lo siento, el servicio de chat no está disponible en este momento. Por favor, llámanos al 1-844-967-3536 para asistencia inmediata.'
            : "I'm sorry, the chat service is unavailable at the moment. Please call us at 1-844-967-3536 for immediate assistance.",
    ...
    try {
      ...
      if (agentResponse.agent !== 'orchestrator' || agentResponse.response) {
        ...
        // Use GHL-based response when OpenAI is not available
        if (quickResponse) {
          aiResponse = quickResponse;
        } else {
          // Create a basic response and trigger GHL campaign
          aiResponse =
            language === 'es'
              ? 'Gracias por tu mensaje. Un miembro de nuestro equipo se pondrá en contacto contigo pronto. Para asistencia inmediata, llama al 1-844-967-3536.'
              : 'Thank you for your message. A member of our team will contact you shortly. For immediate assistance, please call 1-844-967-3536.';
