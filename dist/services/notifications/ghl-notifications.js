...
class GHLNotificationService {
  ...
  async sendSMS(options) {
    try {
      ...
      const result = await gohighlevel_1.ghlService.sendSMSByPhone(
        validated.to,
        validated.body,
  ...
  async sendCaseUpdate(caseInfo) {
    try {
      ...
      if (this.campaignIds.caseUpdate) {
        await gohighlevel_1.ghlService.addToCampaignByPhone(
          caseInfo.clientPhone,
          this.campaignIds.caseUpdate,
          {
  ...
  async sendWelcomeMessage(contact) {
    try {
      if (this.campaignIds.welcomeMessage) {
        await gohighlevel_1.ghlService.addToCampaignByPhone(
          contact.phone,
          this.campaignIds.welcomeMessage,
          {
    ...
    try {
      ...
      if (options.campaignId) {
        ...
        logger_1.logger.warn(
          'No campaign ID provided for call - GoHighLevel requires campaign for calls'
        );
