...
exports.callRouter = exports.CallRoutingService = void 0;
const index_1 = require("./index");
const agent_manager_v2_1 = require("./agent-manager-v2");
...
class CallRoutingService {
    ...
    async routeCall(options) {
        try {
            logger_1.logger.info('Starting call routing', {
            ...
            logger_1.logger.info('Call routing completed', {
                phoneNumber: options.phoneNumber,
            ...
            logger_1.logger.error('Call routing failed:', error);
            // Fallback to general agent
            const fallbackAgentId = await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea('general');
            if (!fallbackAgentId) {
    ...
    async getExistingContactInfo(phoneNumber) {
        try {
            const contact = await gohighlevel_1.ghlService.findContactByPhone(phoneNumber);
            if (contact) {
                return {
                    id: contact.id,
            ...
            logger_1.logger.error('Failed to get existing contact info:', error);
            return null;
        }
    }
    // Determine practice area based on various factors
    async determinePracticeArea(options, existingContact) {
        ...
        if (existingContact?.tags) {
            ...
            if (tags.some((tag) => tag.includes('injury') || tag.includes('accident'))) {
                return 'personal_injury';
            }
            if (tags.some((tag) => tag.includes('criminal') || tag.includes('dui'))) {
                return 'criminal_defense';
            }
            if (tags.some((tag) => tag.includes('workers') || tag.includes('comp'))) {
                return 'workers_compensation';
            }
            if (tags.some((tag) => tag.includes('family') || tag.includes('divorce'))) {
                return 'family_law';
            }
        }
    ...
    async determineLanguage(options, existingContact) {
        ...
        if (existingContact?.tags) {
            const tags = existingContact.tags;
            if (tags.some((tag) => tag.includes('spanish') || tag.includes('español'))) {
                return 'es';
            }
        }
    ...
    async assessUrgency(options, existingContact) {
    ...
    async selectAgent(practiceArea, language, urgency) {
        // For Spanish speakers, try Spanish-specific agents first
        if (language === 'es') {
            const spanishAgentId = await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea(`${practiceArea}_es`);
            if (spanishAgentId) {
                return spanishAgentId;
            }
            // Fallback to general Spanish agent
            const generalSpanishId = await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea('immigration_es');
            if (generalSpanishId) {
                return generalSpanishId;
            }
        }
        // Get practice area specific agent
        const practiceAgentId = await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea(practiceArea);
        if (practiceAgentId) {
            return practiceAgentId;
        }
        // Fallback to general agent
        const generalAgentId = await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea('general');
        if (!generalAgentId) {
    ...
    shouldRequireCallback(urgency, practiceArea, existingContact) {
    ...
    generateSpecialInstructions(params) {
        ...
        if (existingContact) {
            ...
            if (existingContact.lastCallDate) {
                const daysSinceLastCall = Math.floor((Date.now() - new Date(existingContact.lastCallDate).getTime()) / (1000 * 60 * 60 * 24));
    ...
    async createRoutedCall(options) {
        try {
            // Route the call
            ...
            const existingContact = await this.getExistingContactInfo(options.phoneNumber);
            if (existingContact) {
                await gohighlevel_1.ghlService.updateContact(existingContact.id, {
                    customFields: {
            ...
            logger_1.logger.info('Routed call created successfully', {
                callId: call.call_id,
                phoneNumber: options.phoneNumber,
                agentId: routeDecision.agentId,
                priority: routeDecision.priority,
            ...
            logger_1.logger.error('Failed to create routed call:', error);
            throw error;
        }
    }
    // Handle inbound call routing
    async handleInboundCall(phoneNumber, metadata) {
        try {
            ...
            logger_1.logger.error('Failed to handle inbound call:', error);
            throw error;
        }
    }
    // Get routing analytics
    async getRoutingAnalytics(timeRange) {
        try {
            const prisma = (0, prisma_1.getPrismaClient)();
            const where = {};
            if (timeRange) {
            ...
            logger_1.logger.error('Failed to get routing analytics:', error);
            throw error;
        }
    }
}
...
exports.callRouter = CallRoutingService.getInstance();
