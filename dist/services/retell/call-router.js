...
class CallRoutingService {
  ...
  async routeCall(options) {
    try {
      ...
      const fallbackAgentId =
        await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea('general');
      if (!fallbackAgentId) {
  ...
  async selectAgent(practiceArea, language, urgency) {
    ...
    if (language === 'es') {
      const spanishAgentId = await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea(
        `${practiceArea}_es`
      );
      if (spanishAgentId) {
      ...
      const generalSpanishId =
        await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea('immigration_es');
      if (generalSpanishId) {
        return generalSpanishId;
      }
    }
    // Get practice area specific agent
    const practiceAgentId =
      await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea(practiceArea);
    if (practiceAgentId) {
      return practiceAgentId;
    }
    // Fallback to general agent
    const generalAgentId =
      await agent_manager_v2_1.RetellAgentManager.getAgentForPracticeArea('general');
    if (!generalAgentId) {
