...
exports.recordingManager = exports.RecordingManager = void 0;
...
class RecordingManager {
    ...
    async processRecording(callId) {
        try {
            logger_1.logger.info('Processing call recording', { callId });
            ...
            logger_1.logger.info('Recording processed successfully', {
                callId,
            ...
            logger_1.logger.error('Failed to process recording:', error, { callId });
            throw error;
        }
    }
    // Analyze transcript using AI
    async analyzeTranscript(transcript, call) {
        try {
            ...
            analysis.followUpRequired = followUpPatterns.some(pattern => pattern.test(transcript)) || analysis.keywords.includes('callback');
            // Determine urgency
            ...
            logger_1.logger.error('Failed to analyze transcript:', error);
    ...
    generateSummary(transcript, analysis) {
    ...
    async storeRecordingData(data) {
        try {
            ...
            logger_1.logger.info('Recording data stored', {
                callId: data.callId,
            ...
            logger_1.logger.error('Failed to store recording data:', error);
            throw error;
        }
    }
    // Sync recording data to GoHighLevel
    async syncToGHL(callId, data) {
        try {
            // Get GHL contact ID from call metadata
            const ghlContactId = data.call.metadata?.ghlContactId;
            if (!ghlContactId) {
                logger_1.logger.warn('No GHL contact ID found for call', { callId });
                return;
            }
            // Sync recording and transcript
            await gohighlevel_1.ghlService.syncCallRecording(ghlContactId, callId, data.recordingUrl, data.transcript);
            // Update contact with call outcome
            await gohighlevel_1.ghlService.updateContactCallOutcome(ghlContactId, {
                callId,
            ...
            await gohighlevel_1.ghlService.addNote(ghlContactId, noteContent);
            // Update tags based on analysis
            const tags = this.generateTagsFromAnalysis(data.analysis);
            if (tags.length > 0) {
                const contact = await gohighlevel_1.ghlService.getContact(ghlContactId);
                if (contact) {
                    await gohighlevel_1.ghlService.updateContact(ghlContactId, {
                        tags: [...(contact.tags || []), ...tags],
                    });
                }
            }
            logger_1.logger.info('Recording synced to GHL', {
                callId,
                ghlContactId,
            ...
            logger_1.logger.error('Failed to sync recording to GHL:', error);
        }
    }
    // Determine call outcome from analysis
    determineCallOutcome(analysis) {
    ...
    generateTagsFromAnalysis(analysis) {
        const tags = [];
        // Sentiment tags
    ...
    async createFollowUpTasks(callId, analysis, call) {
        try {
            ...
            for (const task of tasks) {
                await gohighlevel_1.ghlService.createTask(task);
            }
            ...
            logger_1.logger.error('Failed to create follow-up tasks:', error);
        }
    }
    ...
    async getContactRecordings(ghlContactId) {
        try {
            const prisma = (0, prisma_1.getPrismaClient)();
            ...
            logger_1.logger.error('Failed to get contact recordings:', error);
            throw error;
        }
    }
    // Get recording analytics
    async getRecordingAnalytics(timeRange) {
        try {
            const prisma = (0, prisma_1.getPrismaClient)();
            const where = {};
            if (timeRange) {
            ...
            recordings.forEach(recording => {
                ...
                // Sentiment
                analytics.sentimentDistribution[recording.sentiment]++;
                // Appointments
                if (recording.appointmentScheduled)
                    appointmentCount++;
                // Follow-ups
                if (recording.followUpRequired)
                    followUpCount++;
                // Keywords
                recording.keywords.forEach((keyword) => {
            ...
            logger_1.logger.error('Failed to get recording analytics:', error);
            throw error;
        }
    }
    ...
    async cleanupOldRecordings(daysToKeep = 90) {
        try {
            ...
            logger_1.logger.info('Old recordings cleaned up', {
            ...
            logger_1.logger.error('Failed to cleanup old recordings:', error);
            throw error;
        }
    }
}
...
exports.recordingManager = RecordingManager.getInstance();
