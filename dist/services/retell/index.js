...
class RetellService {
  ...
  async handleWebhookEvent(event) {
    ...
    try {
      ...
      const { statusManager } = await Promise.resolve().then(() =>
        __importStar(require('./status-manager'))
      );
      const { recordingManager } = await Promise.resolve().then(() =>
        __importStar(require('./recording-manager'))
      );
      const { retellErrorHandler } = await Promise.resolve().then(() =>
        __importStar(require('./error-handler'))
      );
      switch (event.event) {
        case 'call_started':
