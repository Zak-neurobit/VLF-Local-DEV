...
exports.leadValidationAgent = exports.LeadValidationAgent = void 0;
...
const LeadScoreSchema = zod_1.z.object({
  score: zod_1.z.number().min(0).max(100),
  tier: zod_1.z.enum(['hot', 'warm', 'cold', 'invalid']),
  factors: zod_1.z.object({
    urgency: zod_1.z.number().min(0).max(25),
    caseValue: zod_1.z.number().min(0).max(25),
    readiness: zod_1.z.number().min(0).max(25),
    contactQuality: zod_1.z.number().min(0).max(25),
  }),
  recommendations: zod_1.z.array(zod_1.z.string()),
  rejectionReasons: zod_1.z.array(zod_1.z.string()).optional(),
  followUpStrategy: zod_1.z.string(),
  estimatedCaseValue: zod_1.z.number().optional(),
  priorityLevel: zod_1.z.enum(['urgent', 'high', 'medium', 'low']),
  practiceAreas: zod_1.z.array(zod_1.z.string()),
  languagePreference: zod_1.z.enum(['en', 'es', 'bilingual']),
  validationTimestamp: zod_1.z.string(),
  ghlContactId: zod_1.z.string().optional(),
});
class LeadValidationAgent extends base_1.Agent {
  constructor() {
    super({
  ...
  async validateLead(leadData) {
    try {
      ...
      logger_1.logger.error('Lead validation error:', error);
      throw error;
    }
  }
  ...
  calculateCaseValueScore(leadData) {
    let score = 0;
    const message = leadData.message.toLowerCase();
  ...
  calculateContactQualityScore(leadData) {
    let score = 0;
  ...
  determineFollowUpStrategy(tier, priority) {
  ...
  identifyPracticeAreas(leadData) {
    const message = leadData.message.toLowerCase();
    const areas = [];
  ...
  detectLanguagePreference(leadData) {
  ...
  async updateGHLContact(leadData, validation) {
    try {
      ...
      logger_1.logger.error('Error updating GHL contact:', error);
      throw error;
    }
  }
  async logValidation(leadData, validation) {
    try {
      ...
      logger_1.logger.info('Lead validation completed:', {
        email: leadData.email,
        score: validation.score,
        tier: validation.tier,
      ...
      logger_1.logger.error('Error logging validation:', error);
    }
  }
  async execute(input) {
...
exports.LeadValidationAgent = LeadValidationAgent;
// Export singleton instance
exports.leadValidationAgent = new LeadValidationAgent();
