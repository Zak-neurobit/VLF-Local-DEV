...
async function verifyDynamicFixes() {
  ...
  for (const routePath of routeFiles) {
    try {
      ...
      const check: RouteCheck = {
        ...
        usesSearchParams: content.includes('searchParams') || content.includes('nextUrl.searchParams'),
        dynamicFeatures: []
      };
  ...
  console.log(`⚠️  May have unnecessary dynamic: ${results.filter(r => r.hasDynamic && r.dynamicFeatures.length === 0).length}`);
  console.log('='.repeat(60));
