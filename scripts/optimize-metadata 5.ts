...
function detectPageType(filePath: string): { type: string; subType?: string; variables?: Record<string, string> } {
  const relativePath = path.relative(APP_DIR, filePath).toLowerCase();
  
  // Practice areas
  if (relativePath.includes('practice-areas')) {
    if (relativePath.includes('immigration')) return { type: 'practiceArea', subType: 'immigration' };
    if (relativePath.includes('personal-injury')) return { type: 'practiceArea', subType: 'personalInjury' };
    if (relativePath.includes('criminal-defense')) return { type: 'practiceArea', subType: 'criminalDefense' };
    if (relativePath.includes('workers-compensation')) return { type: 'practiceArea', subType: 'workersComp' };
  }
  ...
  if (relativePath.includes('neighborhoods')) {
    ...
    if (parts[neighborhoodIndex]) {
      return {
        ...
        variables: { 
          neighborhood: parts[neighborhoodIndex].replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
          city: parts[neighborhoodIndex - 1]?.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) || 'NC'
        }
      };
    }
  }
  ...
  if (relativePath.includes('near-me')) {
    ...
    if (match) {
      ...
      if (nearMeIndex > 0) {
        const city = parts.slice(0, nearMeIndex).join(' ').replace(/\b\w/g, l => l.toUpperCase());
        const service = parts.slice(nearMeIndex + 2).join(' ').replace(/\b\w/g, l => l.toUpperCase());
        return {
          type: 'location',
          subType: 'nearMe',
...
function extractCurrentMetadata(content: string): { title?: string; description?: string; type: 'static' | 'dynamic' } {
...
async function updateMetadataInFile(filePath: string, updates: MetadataUpdate): Promise<boolean> {
  try {
    ...
    if (!content.includes('metadata-optimizer')) {
      const importStatement = "import { optimizeMetadata, generateOptimizedTitle, generateOptimizedDescription } from '@/lib/seo/metadata-optimizer';\n";
...
async function optimizeAllMetadata() {
  ...
  for (const file of files) {
    ...
    if (currentMeta.type === 'static' && currentMeta.title) {
      ...
      update.suggestedDescription = generateOptimizedDescriptionInline(pageInfo, currentMeta.description);
      
      // Only update if suggestions are different and better
      if (update.suggestedTitle !== update.currentTitle || 
...
function generateOptimizedTitleInline(pageInfo: any, currentTitle: string): string {
  ...
  if (pageInfo.type === 'practiceArea') {
    ...
    const aggressionWord = powerWords.aggression[Math.floor(Math.random() * powerWords.aggression.length)];
  ...
  if (pageInfo.type === 'location' && pageInfo.variables?.city) {
    ...
    return currentTitle.replace(pageInfo.variables.city, `${pageInfo.variables.city} ${urgencyWord}`);
  }
