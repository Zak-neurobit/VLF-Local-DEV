...
class CrewAIDemo {
  ...
  private async showRealTimeMonitoring(): Promise<void> {
    ...
    logger.info(
      `  ⚡ Parallel Processing: ${systemStatus.parallelProcessing ? 'ENABLED' : 'DISABLED'}`
    );
    ...
    recentLogs.forEach((log, index) => {
      ...
      logger.info(
        `  ${index + 1}. ${log.agentName} - ${log.executionType} (${timeAgo}s ago) ${log.success ? '✅' : '❌'}`
      );
    });
