...
async function testSEOImplementation() {
  ...
  for (const file of seoFiles) {
    ...
    if (exists) {
      ...
      if (content) {
        if (
          file.name === 'Sitemap Generator' &&
          content.includes('export default function sitemap')
        ) {
...
${
  testResults.tests.pages.details.filter(d => !d.exists).length > 0
    ? `
**Missing Pages**:
${testResults.tests.pages.details
  .filter(d => !d.exists)
  .map(d => `- ${d.name}: ${d.path}`)
  .join('\n')}
`
...
${testResults.tests.spanish.details
  .filter(d => !d.exists)
  .map(d => `- ${d.name}: ${d.path}`)
  .join('\n')}
`
...
${testResults.tests.chatbot.details
  .filter(d => !d.exists)
  .map(d => `- ${d.name}: ${d.path}`)
  .join('\n')}
`
...
${
  testResults.tests.agents.details
    ? testResults.tests.agents.details.filter(d => !d.exists).length > 0
      ? `
...
${
  testResults.tests.api.details
    ? testResults.tests.api.details.filter(d => !d.exists).length > 0
      ? `
...
async function runTests() {
  try {
    ...
    log(
      `Failed: ${testResults.summary.totalFailed} (${((testResults.summary.totalFailed / testResults.summary.totalTests) * 100).toFixed(1)}%)`,
      testResults.summary.totalFailed > 0 ? 'error' : 'success'
    );
    ...
    if (testResults.summary.totalFailed > 0) {
      ...
      log(
        'Next: Run `npm run dev` and use comprehensive-site-test.js for runtime testing.',
        'info'
      );
    }
  } catch (error) {
