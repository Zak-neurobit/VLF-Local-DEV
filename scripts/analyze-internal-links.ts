...
async function analyzeInternalLinks(): Promise<LinkAnalysis> {
  ...
  for (const file of files) {
    ...
    linkPatterns.forEach(pattern => {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        if (href.startsWith('/') && !href.startsWith('//')) {
          ...
          const anchorMatch = content.substring(match.index, match.index + 200)
            .match(/>(.*?)</);
          if (anchorMatch) {
    ...
    analysis.linkDistribution[bucketLabel] = 
      (analysis.linkDistribution[bucketLabel] || 0) + 1;
  }
...
function generateRecommendations(
  ...
  if (overusedAnchors.length > 0) {
    analysis.recommendations.push(
      ...
      overusedAnchors.slice(0, 3).map(([text, count]) => `"${text}" (${count}x)`).join(', ')
    );
  }
  ...
  if (hubPages.length > 0) {
    analysis.recommendations.push(
      ...
      hubPages.slice(0, 3).map(([url, count]) => `${url} (${count} links)`).join(', ')
    );
  }
...
async function generateReport() {
  try {
    ...
    console.log(`   Average Links per Page: ${(analysis.totalInternalLinks / analysis.totalPages).toFixed(1)}`);
    console.log('');
