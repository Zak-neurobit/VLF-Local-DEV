...
async function testI18nSetup() {
  ...
  try {
    ...
    if (nextConfig.i18n && nextConfig.i18n.locales.includes('es')) {
      tests.push({ test: 'Next.js i18n config', status: '✅', details: 'Configured with locales: ' + nextConfig.i18n.locales.join(', ') });
    } else {
      tests.push({ test: 'Next.js i18n config', status: '❌', details: 'i18n not properly configured' });
    }
  } catch (error) {
  ...
  for (const file of translationFiles) {
    try {
      ...
      tests.push({ test: `Translation file: ${file}`, status: '✅', details: `${keyCount} translation keys found` });
    } catch (error) {
  ...
  try {
    ...
    tests.push({ test: 'Middleware file', status: '✅', details: 'Middleware exists for language routing' });
  } catch {
  ...
  try {
    ...
    tests.push({ test: 'LanguageSwitcher component', status: '❌', details: 'Component not found' });
  }
