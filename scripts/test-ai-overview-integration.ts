...
import { aiOverviewTrainer, validateContentForAIOverview } from '../src/lib/crewai/training/ai-overview-training-module';
  ...
  private async testAIOverviewContentGeneration(): Promise<void> {
    ...
    for (const testCase of testCases) {
      try {
        ...
        const hasVoiceOptimization = result.faqSection.questions.every(
        ...
        const passed = hasFAQs && hasOptimalAnswers && hasVoiceOptimization && 
                      meetsReadinessThreshold && meetsQualityThreshold;

        this.results.push({
          testName: `AI Overview Generation - ${testCase.practiceArea}`,
          passed,
          score: (result.aiOverviewMetrics.readinessScore + result.aiOverviewMetrics.answerQuality) / 2,
...
`;
    ...
    try {
      const voiceOptimization = await voiceSearchOptimizer.optimizeForVoiceSearch(
        testContent,
        {
          practiceArea: 'immigration',
          contentType: 'article',
      ...
      const passed = hasConversationalHeadings && hasNaturalQueries && 
                    hasDirectAnswers && hasLocalVariations;
  ...
  private async testSchemaMarkupGeneration(): Promise<void> {
    ...
    try {
      ...
      const faqData = [
        {
          ...
          answer: 'Green card processing times vary by category, typically taking 8-33 months for family-based applications.',
        },
        {
          question: 'What documents do I need for citizenship?',
          answer: 'You need your green card, tax returns, travel records, and other supporting documents for N-400 application.',
        },
      ];
      ...
      const hasMainEntity = Array.isArray(faqSchema.mainEntity) && faqSchema.mainEntity.length === 2;
      ...
      const hasAllSchemas = aiOverviewResult.schemaMarkup.faqSchema && 
                           aiOverviewResult.schemaMarkup.legalServiceSchema;
  ...
  private async testPracticeAreaTraining(): Promise<void> {
    ...
    const practiceAreas = ['immigration', 'personal-injury', 'workers-compensation', 'family-law', 'criminal-defense'];
    ...
    for (const practiceArea of practiceAreas) {
      try {
        ...
        const hasSchemaRequirements = Object.values(trainingData.schemaRequirements).some(req => req);
        
        const passed = hasCommonQuestions && hasOptimalAnswers && 
                      hasVoiceOptimization && hasSchemaRequirements;
...
`,
    ...
    for (const testContent of testContents) {
      try {
        const validation = await validateContentForAIOverview(
          testContent.content,
          'immigration'
        );
  ...
  private async testPerformanceBenchmarks(): Promise<void> {
    ...
    try {
      ...
      await voiceSearchOptimizer.optimizeForVoiceSearch(
        'Sample content for voice search testing',
        {
          practiceArea: 'immigration',
          contentType: 'article',
          targetAudience: 'potential_clients',
  ...
  private getMockTrainingData(practiceArea: string) {
    return {
      ...
      competitiveIntelligence: {
        ...
        differentiationOpportunities: ['Local expertise', 'Specialized knowledge', 'Client success stories'],
      },
    };
  }
}
