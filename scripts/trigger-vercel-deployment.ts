    ...
    const checkDeployment = async () => {
      ...
      try {
        ...
        switch (status.readyState) {
          ...
          default:
            process.stdout.write(`\r${colors.yellow}⏳${colors.reset} Status: ${status.readyState} (${checkCount}/${maxChecks})`);
...
async function createDeploymentHook() {
  ...
  try {
    ...
    // Save hook URL
    fs.writeFileSync(
      path.join(process.cwd(), '.vercel', 'deploy-hook.txt'),
      hook.url
    );
