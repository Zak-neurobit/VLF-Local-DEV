...
async function generateReport(allResults) {
  ...
  const avgDesktopScore = allResults.reduce((sum, r) => sum + r.desktop.criticalMetrics.score, 0) / allResults.length;
  const avgMobileScore = allResults.reduce((sum, r) => sum + r.mobile.criticalMetrics.score, 0) / allResults.length;
...
async function main() {
  ...
  for (const page of PAGES_TO_TEST) {
    ...
    try {
      ...
      const desktopPath = path.join(reportsDir, `${timestamp}-${page.name.toLowerCase().replace(/\s+/g, '-')}-desktop`);
      ...
      const mobilePath = path.join(reportsDir, `${timestamp}-${page.name.toLowerCase().replace(/\s+/g, '-')}-mobile`);
      ...
      console.log(`  ✅ ${page.name} - Desktop: ${desktopAnalysis.criticalMetrics.score}, Mobile: ${mobileAnalysis.criticalMetrics.score}\n`);
    } catch (error) {
  ...
  console.log('\n📊 Quick Summary:');
  allResults.forEach(({ page, desktop, mobile }) => {
    const desktopEmoji = desktop.criticalMetrics.score >= 90 ? '✅' : desktop.criticalMetrics.score >= 50 ? '⚠️' : '❌';
    const mobileEmoji = mobile.criticalMetrics.score >= 90 ? '✅' : mobile.criticalMetrics.score >= 50 ? '⚠️' : '❌';
    console.log(`${page}: Desktop ${desktopEmoji} ${desktop.criticalMetrics.score}/100 | Mobile ${mobileEmoji} ${mobile.criticalMetrics.score}/100`);
  });
}
